async function fetchMatches() {
    const response = await fetch('http://127.0.0.1:8000/api/users/get/historic', {method: 'GET', credentials: 'include'});
    return response.json();
}


function get_status_matche(is_winner) {
    if (is_winner) {
        return `
...
const Matches = async(state) => {
    ...
    console.log(matches)
    matches = matches.reduce((acc, item) => {return acc + `
        <tr class="table-row">
