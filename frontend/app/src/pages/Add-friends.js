<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8" />
    <link rel="stylesheet" href="globals.css" />
    <link rel="stylesheet" href="styleguide.css" />
    <link rel="stylesheet" href="style.css" />
  </head>
  <body>
    <div class="adicionar-amigo">
      <div class="frame">
        <div class="div">
          <div class="div-wrapper"><div class="text-wrapper">Add Friend</div></div>
          <div class="icon-wrapper"><img class="icon" src="img/image.svg" /></div>
        </div>
        <div class="search-bar">
          <div class="icon-wrapper"><img class="img" src="img/icon.svg" /></div>
          <div class="search-bar-2">Search for a name...</div>
        </div>
      </div>
      <div class="frame-2">
        <div class="frame-3">
          <div class="text-wrapper-2">PLAYER</div>
          <div class="text-wrapper-2">GLOBAL RANKING</div>
          <div class="text-wrapper-3">ACTIONS</div>
        </div>
        <div class="frame-2">
          <div class="div-2">
            <div class="table-row-player">
              <div class="table-row-player-2"></div>
              <div class="table-row-player-3">
                <div class="table-row-player-4">Caos</div>
                <div class="table-row-player-5">clourenc</div>
              </div>
            </div>
            <div class="table-row-data"><div class="text-wrapper-4">#1</div></div>
            <div class="table-row-actions">
              <button class="button">
                <div class="button-types"><div class="button-text">Add friend</div></div>
              </button>
            </div>
          </div>
          <div class="div-2">
            <div class="table-row-player">
              <div class="table-row-player-6"></div>
              <div class="table-row-player-3">
                <div class="table-row-player-4">Caos</div>
                <div class="table-row-player-5">clourenc</div>
              </div>
            </div>
            <div class="table-row-data"><div class="text-wrapper-4">#1</div></div>
            <div class="table-row-actions">
              <button class="button">
                <div class="button-types"><div class="button-text">Add friend</div></div>
              </button>
            </div>
          </div>
          <div class="div-2">
            <div class="table-row-player">
              <div class="table-row-player-7"></div>
              <div class="table-row-player-3">
                <div class="table-row-player-4">Caos</div>
                <div class="table-row-player-5">clourenc</div>
              </div>
            </div>
            <div class="table-row-data"><div class="text-wrapper-4">#1</div></div>
            <div class="table-row-actions">
              <button class="button">
                <div class="button-types"><div class="button-text">Add friend</div></div>
              </button>
            </div>
          </div>
          <div class="div-2">
            <div class="table-row-player">
              <div class="table-row-player-8"></div>
              <div class="table-row-player-3">
                <div class="table-row-player-4">Caos</div>
                <div class="table-row-player-5">clourenc</div>
              </div>
            </div>
            <div class="table-row-data"><div class="text-wrapper-4">#1</div></div>
            <div class="table-row-actions">
              <button class="button">
                <div class="button-types"><div class="button-text">Add friend</div></div>
              </button>
            </div>
          </div>
        </div>
      </div>
    </div>
  </body>
</html>
